import { BadgeDollarSignIcon, Calendar, CalendarIcon, ChevronRightIcon, FileTextIcon, icons, LayoutGridIcon, LogOutIcon, MenuIcon, SettingsIcon, UserIcon, XIcon } from 'lucide-react'
...
const Sidebar = () => {
    ...
    const navItems = [
        ...
        { name: "Payslips", href: "/payslips", icon: BadgeDollarSignIcon },
    ...
    const sidebarContent = (
        <>
            ...
            <div className='px-5 pt-6 pb-5 border-b border-white/6'>
                <div className='flex items-center justify-between'>
                    ...
                    <button onClick={() => setMobileOpen(false)}
                        className='lg:hidden text-slate-400 hover:text-whitep-1'>
                        <XIcon size={20} />
                    </button>
                </div>
            </div>
            ...
            {/*User Profile Card*/}
            {userName && (
                <div className='mx-3 mt-4 mb-1 p-3 rounded-1g bg-white/3 border border-white/4'>
                    <div className='flex items-center gap-3'>
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <Link
                            ...
                            {/* Fixed: className & -translate-y-1/2 */}
                            {isActive && (
                                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full bg-indigo-500" />
                            )}

                            {/* Fixed: removed spaces from custom sizes and modifiers */}
                            <item.icon
            ...
            <div className="p-3 border-t border-white/6">
                <button onClick={handleLogout} className='flex items-centergap-3 w-full px-3 py-2.5 rounded-md text-[13px] font-medium text-slate-400 hover: text-rose-400 hover:bg-rose-500/8 transition-all duration-150'>
    ...
    return (
        <>
            ...
            {/*Sidebar - desktop*/}
            <aside className='hidden lg:flex flex-col h-full w-64 bg-slate-900 text-white shrink-0 border-r border-white/5'>
                {sidebarContent}
            </aside>
